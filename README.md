...
> 🙏 **커뮤니티 의견 반영**
> - **v2.0.5 (현재)** — 스레드 사용자 **@imusiro** 님의 *"저장 파일은 어디서 받나요?"* 의견 반영. 이제 **다운로드만 받으면 끝** (직접 만들 필요 X).
> - **v2.0.6 (다음 릴리즈)** — **@akra.dev** 님의 *"맥북은 Caps Lock 이 기본 한/영 변환인데 맥 환경은 안 좋아하시나봄"* 의견 반영 예정. **Caps Lock 키 = 한/영 토글** 기본으로 박을 예정. (지금 v2.0.5 도 한 줄로 활성화 가능 → [QnA Q · 맥북 Caps Lock](#4-한글--와이파이) 참조)
...
> 🙏 **@akra.dev** 님 의견 반영 — **다음 버전 v2.0.6** 에서 **Caps Lock = 한/영** 기본으로 박을 예정.
